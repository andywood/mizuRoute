...
CONTAINS
 ...
 SUBROUTINE inFile_pop(ierr, message)  ! output
  ...
  integer(i4b)                         :: nTime, nt             ! number of time step per file and all files, respectively
  character(len=strLen)                :: cmessage              ! error message of downwind routine
  ...
  do iFile=1,nFile
   ...
   print*, 'iFile, nFile, infileinfo_data(iFile)%infilename = ', iFile, nFile, trim(infileinfo_data(iFile)%infilename)
...
!   print*, "calendar is ", infileinfo_data(iFile)%calendar
   ...
   ! Compute reference Julian day for each nc file
   call process_time(trim(infileinfo_data(iFile)%unit), infileinfo_data(iFile)%calendar,  infileinfo_data(iFile)%refJulday, ierr, cmessage)
  ...
  refJulday     = infileinfo_data(1)%refJulday
  convTime2Days = infileinfo_data(1)%convTime2Days

  ! find the total length of the timeVar
  nt = 0; ! counter
  do iFile=1,nFile
   nt = nt + infileinfo_data(iFile)%nTime
  enddo
!  print*, "size of the time var: ", counter ! the total number of time steps in input files from first to last

  ! allocate the timeVar
  allocate(timeVar(nt), roJulday(nt), stat=ierr) ! what is the allocate stat?

  ! pass the time var from each file into the global time var
  counter = 1; ! counter
  do iFile=1,nFile
   nTime = infileinfo_data(iFile)%nTime
   roJulday(counter:counter+nTime-1) = infileinfo_data(iFile)%refJulday + infileinfo_data(iFile)%timeVar(1:nTime)/infileinfo_data(iFile)%convTime2Days
   counter = counter + nTime
  enddo
  timeVar(1:nt) = (roJulday(1:nt) - refJulday)*convTime2Days
